...
PanelWindow {
    ...
    property var    micSources: []        // [{id, name}, …] from the daemon
    property string micSelected: ""       // description name of chosen input
    property bool   micMenuOpen: false
    onShownChanged: if (shown) send({ type: "list_mics" })
    onLinkUpChanged: if (linkUp) send({ type: "list_mics" })
        ...
        Socket {
            ...
            parser: SplitParser {
                onRead: line => {
                    ...
                    case "sources":
                        root.micSources = m.items
                        root.micSelected = m.selected
                        break
    ...
    Rectangle {
        ...
        Column {
            ...
            Item {
                ...
                Row {
                    ...
                    // mic selector — click to choose the audio input
                    Rectangle {
                        width: micBtnRow.width + 18; height: 22; radius: 11
                        color: root.micMenuOpen ? "#223a5cff" : "transparent"
                        border.color: root.micMenuOpen ? root.cBlue : root.cDim
                        border.width: 1
                        anchors.verticalCenter: parent.verticalCenter
                        Row {
                            id: micBtnRow
                            ...
                            Text {
                                text: root.micSelected === "" ? "auto" : root.micSelected
                                color: root.cText
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                if (root.micMenuOpen) root.send({ type: "list_mics" })
                            }
                        }
                    }
                }
        ...
        // ── audio-input dropdown (overlays the card) ──────────────────
        Rectangle {
            ...
            y: 56
            width: 360
            height: Math.min(micList.contentHeight + 40, 260)
            color: "#f5101010"
            ...
            Column {
                ...
                ListView {
                    ...
                    delegate: Rectangle {
                        ...
                        height: 32
                        radius: 6
                        color: hov.containsMouse ? "#223a5cff"
                             : (modelData.name === root.micSelected ? "#143a5cff" : "transparent")
                        Row {
                            anchors { left: parent.left; verticalCenter: parent.verticalCenter; leftMargin: 8 }
                            spacing: 8
                            Text {
                                text: modelData.name === root.micSelected ? "" : ""
                                color: root.cBlue
                                width: 14
                                anchors.verticalCenter: parent.verticalCenter
                        ...
                        MouseArea {
                            ...
                            cursorShape: Qt.PointingHandCursor
                            onClicked: {
                                root.send({ type: "set_mic", name: modelData.name })
                                root.micSelected = modelData.name
