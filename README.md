...
- **[feathershot](https://github.com/r3dg0d/feathershot) MKII** — a native Wayland screenshot + annotation tool, bound to `PrtSc` in [`config/niri/config.kdl`](config/niri/config.kdl) in place of niri's built-in screenshot. `grim`/`slurp` capture → a **Quickshell overlay** ([`config/quickshell/Feathershot.qml`](config/quickshell/Feathershot.qml) here, canonical copy [upstream](https://github.com/r3dg0d/feathershot/blob/main/qml/Feathershot.qml)) for arrows, boxes, circles, lines, freehand and text → composited back onto the original capture with cairo at native resolution. MKII moved the editor out of a per-screenshot GTK window and into the always-running `qs`, so it appears the instant you release the selection; shapes are tracked in image coordinates, so a scaled-down preview still saves full-size.
