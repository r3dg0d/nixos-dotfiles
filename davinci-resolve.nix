    ...
    stdenv.mkDerivation rec {
      ...
      src =
        runCommandLocal "${pname}-src.zip"
          rec {
            ...
            REFERID = "a43a1c175b89473c993229cf02750dea";
      ...
      installPhase =
        ...
        ''
          ...
          mkdir -p $out
          echo "appimageName is $appimageName"
          test -e ${lib.escapeShellArg appimageName}
          echo "running appimage-exec.sh -x $out ${lib.escapeShellArg appimageName}"
          appimage-exec.sh -x $out ${lib.escapeShellArg appimageName}

          echo "copying files from $out to $out"
          ...
          echo "verifying all three rules files are present"
